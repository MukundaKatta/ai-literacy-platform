// ai-literacy-platform — sandbox module
...
export class Sandbox extends EventEmitter {
  ...
  constructor(options: SandboxOptions = {}) {
    super();
    this.options = { name: options.name ?? "Sandbox", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<SandboxResult> {
